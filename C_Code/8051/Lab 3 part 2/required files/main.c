...
/***************************************************************************************************************************
 *    File name   : main.c
 *    Description : ESD lab 3 part 2 sdcc code. This code first accepts an input from the user. Then creates 2 buffers
 *                  (BUFFER_0 and BUFFER_1). After creating buffers, accept characters inputs from the user and store them in
 *                  buffer 0 and perform the remaining functions (+,-,?,=,@) according to user input and the requirement
 *                  of the lab.
 *
...
//               DECLARING VARIABLES
    ...
    do
    {
        ...
        int divisible= number%16;

        if((number<48 || number>4800))
        {
...
ishaan:
        ...
        //Take a input for storage in buffer 0. This character has to be between a-z.
        input_char=getchar();
        total_char++;
        putchar(input_char);
        printf_tiny("\n\r\n\r");
